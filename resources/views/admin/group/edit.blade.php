							...
							<div class="form-check">
								<div class="input-group col-md-12">
									...
									<div class="input-group col-md-4">
										...
										<input type="text" value="{{$info->groupnumber}}" name="groupnumber" class="form-control">
										<div class="input-group-append">
										</div>
									</div>
									<div class="input-group col-md-4">
										...
										<input type="text" value="{{$info->entdate}}" name="entdate" class="form-control" id="test1">
										<div class="input-group-append">
										</div>
									</div>
								</div>
							...
							<table class="table table-hover">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											@if($v->status)
												<span class="btn btn-success" onclick="status(this,{{$v->id}},1)">已支付</span>
											@else
												<span class="btn btn-warning" onclick="status(this,{{$v->id}},0)">未支付</span>
											@endif
										</td>
									</tr>
									@endforeach
...
function status (obj,id,status){
	if(status){
		//发送ajax请求
		$.post('../../../admin/group/ajaxStatu',{id:id,"_token":"{{csrf_token()}}","status":"0"},function(data){
			if (data==1){
				$(obj).parent().html('<span class="btn btn-warning" onclick="status(this,'+id+',0)">未支付</span>')
			}else{
				alert('修改失败');
			}
		})
	}else{
		$.post('../../../admin/group/ajaxStatu',{id:id,"_token":"{{csrf_token()}}","status":"1"},function(data){
			if (data==1){
				$(obj).parent().html('<span class="btn btn-success" onclick="status(this,'+id+',1)">已支付</span>')
			}else{
				alert('修改失败');
			}
		})
